...
class GitHubMCPTester {
  ...
  async testToolDefinition(toolName, expectedDescription = null) {
    ...
    try {
      ...
      if (!tool) {
        this.failedTests++;
        this.log(`Tool '${toolName}' not found in tools configuration`, 'error');
        return false;
      }
      ...
      if (expectedDescription && !tool.description.toLowerCase().includes(expectedDescription.toLowerCase())) {
        this.failedTests++;
        this.log(`Tool '${toolName}' description doesn't match expected content`, 'warning');
        return false;
      }
      ...
      this.failedTests++;
      this.log(`Error testing tool definition '${toolName}': ${error.message}`, 'error');
      return false;
    }
  }
  ...
  async testHandlerMethod(toolName, expectedMethod) {
    ...
    try {
      ...
      if (toolName.includes('repo') && !toolName.includes('label') && !toolName.includes('milestone')) {
      ...
      if (typeof handler[methodName] !== 'function') {
        this.failedTests++;
        this.log(`Method '${methodName}' not found in handler for tool '${toolName}'`, 'error');
        return false;
      }

      this.passedTests++;
      this.log(`Handler method '${methodName}' exists for tool '${toolName}'`, 'success');
      return true;
    } catch (error) {
      this.failedTests++;
      this.log(`Error testing handler for '${toolName}': ${error.message}`, 'error');
      return false;
    }
  }
  ...
  async testSwitchCase(toolName) {
    ...
    try {
      ...
      try {
        const callHandler = server.server.requestHandlers.get(CallToolRequestSchema);
        await callHandler({
          params: {
            name: toolName,
            arguments: {}
        ...
        if (error.message.includes(`Unknown tool: ${toolName}`)) {
          ...
          this.passedTests++;
          this.log(`Switch case exists for tool '${toolName}' (failed as expected)`, 'success');
          return true;
        }
      }
    } catch (error) {
      this.failedTests++;
      this.log(`Error testing switch case for '${toolName}': ${error.message}`, 'error');
      return false;
    }
  }
  ...
  async runAllTests() {
    ...
    const toolsToTest = [
      ...
      { name: 'remove_assignees_from_issue', method: 'removeAssigneesFromIssue' },
    ...
    for (const [toolName, toolDef] of Object.entries(toolsConfig)) {
      ...
      if (missingProps.length === 0) {
        ...
        this.failedTests++;
        this.log(`Tool '${toolName}' missing properties: ${missingProps.join(', ')}`, 'error');
      }
    }
  ...
  printResults() {
    ...
    if (this.failedTests === 0) {
      this.log("\n🎉 All tests passed! GitHub MCP Server is fully functional!", 'success');
    } else {
      this.log(`\n⚠️  ${this.failedTests} test(s) failed. Check the errors above.`, 'warning');
    }
    ...
    console.log("For runtime testing with GitHub API, use a tool like MCP Inspector or set up integration tests.");
  }
}
