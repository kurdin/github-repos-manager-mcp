...
const serverPath = path.join(__dirname, 'src', 'index.cjs');
      ...
      try {
        ...
        const responseLine = lines.find(line => {
          try {
            ...
            return parsed.jsonrpc === '2.0' && parsed.id === testCase.command.id;
          } catch {
            return false;
          }
        });
        ...
        if (responseLine) {
          ...
          if (response.result) {
            console.log('Success!');
            console.log('Result preview:', JSON.stringify(response.result, null, 2).substring(0, 500) + '...');
